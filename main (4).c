...
bool exprPrimary(Ret *r){
    ...
    if(consume(CT_INT)){*r = (Ret){ {TB_INT,NULL,-1},false,true }; return true;}
    if(consume(CT_REAL)){ *r = (Ret){ {TB_DOUBLE,NULL,-1},false,true }; return true;}
...
void run(){
	...
	void(*extFnPtr)();
	for(;;){ // afiseaza indexul instructiunii curente si numarul de valori din stiva
		...
		switch(IP->op){
			...
			case OP_RET_VOID: iArg=IP->arg.i; printf("RET_VOID\t%d",iArg);
				IP=FP[-1].p; SP=FP-iArg-2; FP=FP[0].p; break;
			case OP_JMP: printf("JMP\t%d",IP->arg.i); IP=instructions+IP->arg.i;
				break;
			case OP_JF: iTop=popi(); printf("JF\t%d\t// %d",IP->arg.i,iTop);
				IP=iTop ? IP+1 : instructions+IP->arg.i; break;
...
/**
    addInstrWithDouble(OP_PUSH_F,2);
	int callPos=addInstr(OP_CALL);
	addInstr(OP_HALT);
	instructions[callPos].arg.i=nInstructions;
	addInstrWithInt(OP_ENTER,1);
	// i=0;  (componentele lui for sunt implementate sub forma unui while)
	addInstrWithDouble(OP_PUSH_F,0);
	addInstrWithInt(OP_FPSTORE,1);
	// while(i<n){
	int whilePos=addInstrWithInt(OP_FPLOAD,1);
	addInstrWithInt(OP_FPLOAD,-2);
